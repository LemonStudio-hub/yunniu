...
export class PostService {
  ...
  async create(input: CreatePostInput): Promise<Post> {
    ...
    await this.db
      .prepare(
        'INSERT INTO posts (id, title, content, author_id, category_id, status, reason) VALUES (?, ?, ?, ?, ?, ?, ?)'
      )
      .bind(id, input.title, input.content, input.author_id, input.category_id, input.status || 'pending', input.reason || null)
      .run()
  ...
  async findPendingPosts(options: {
    ...
    const query = `
      ...
      LEFT JOIN users u ON p.author_id = u.id AND u.deleted_at IS NULL
      LEFT JOIN categories c ON p.category_id = c.id
      WHERE p.deleted_at IS NULL AND p.status IN ('pending', 'appealed')
      ORDER BY p.created_at DESC LIMIT ? OFFSET ?
    `

    const postsResult = await this.db.prepare(query).bind(limit, offset).all()
    ...
    const countResult = await this.db
      .prepare('SELECT COUNT(*) as count FROM posts WHERE deleted_at IS NULL AND status IN (?, ?)')
      .bind('pending', 'appealed')
      .first<{ count: number }>()
  ...
  async approvePost(id: string): Promise<Post | null> {
    await this.db
      .prepare('UPDATE posts SET status = ?, reason = NULL, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
      .bind('approved', id)
      .run()
  ...
  async rejectPost(id: string, reason: string): Promise<Post | null> {
    await this.db
      .prepare('UPDATE posts SET status = ?, reason = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
  ...
  async appealPost(id: string, appealReason: string): Promise<Post | null> {
    await this.db
      .prepare('UPDATE posts SET status = ?, reason = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?')
      .bind('appealed', appealReason, id)
      .run()
